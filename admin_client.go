...
import (
	...
	"github.com/zhanchangbao/hbasegosdk/zk/region"
	"github.com/zhanchangbao/hbasegosdk/zk/zk"
)
...
type AdminClient interface {
	...
	ListProcedure() ([]*pb.Procedure, error)
}
...
//Get the procedure of the cluster
func (c *client) ListProcedure() ([]*pb.Procedure, error) {
	pbmsg, err := c.SendRPC(hrpc.NewListProcedure())
	if err != nil {
		return nil, err
	}
	r, ok := pbmsg.(*pb.ListProceduresResponse)
	if !ok {
		return nil, fmt.Errorf("sendRPC returned not a ListProceduresResponse")
	}
